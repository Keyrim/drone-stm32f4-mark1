...
void SIMULATION_CLOSED_LOOP_Main(high_level_t * high_level)
{
	...
	else
	{
		...
		high_level->target_angle_speed[axe_eROLL] = ((float)high_level->radio[1] - 1500.0f)*0.05f;
		high_level->target_angle_speed[axe_eROLL] = ((float)high_level->radio[3] - 1500.0f)*0.05f;
